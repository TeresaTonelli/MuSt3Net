...
def compute_ensemble_std(list_path_test_loss):
    ...
    return np.mean(np.array(n_ens_means))    #qua io tornerei una media delle std, non la std delle std
...
def compute_3D_ensemble_mean_std(test_data, list_ensemble_models, path_mean_std):
    ...
    for ens_model in list_ensemble_models:
        print("type ens model", type(ens_model))
        #print("ens model on cuda?", ens_model.get_device())
        ens_model.to(device)
        #print("ens model on cuda?", ens_model.get_device())
        ens_model.eval()
        with torch.no_grad():
    ...
    print("stacked tensor shape", stacked_tensor_outputs.shape)
    print("check correct stacking", torch.equal(stacked_tensor_outputs[0, :, :, :, :, :], list_models_outputs[0]))
    tensor_mean = torch.mean(stacked_tensor_outputs, dim=0)
    tensor_std = torch.std(stacked_tensor_outputs, dim=0)    #sembra dovrebbe funzionare allo stesso modo della media
