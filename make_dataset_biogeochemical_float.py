...
def insert_float_values(lat_limits, lon_limits, depth_limits, year_limits, resolution, list_data_times, dict_list_parallelepiped_vars, list_float_vars):
    ...
    for i in range(np.size(list_data)):  # indexing on list_data and list_datetime also
        ...
        index = list_data_times[int(time) - int(year_interval[0])].index(time)  # index input tens considered, i.e. the one to upd

        for biogeoch_var in list_biogeoch_vars:

          print("biogeoch var float", dict_float_names[biogeoch_var], flush=True)
          if dict_float_names[biogeoch_var] in list_float_vars[i]:
            ...
            print("select par init", np.count_nonzero(select_parallelepiped))
            print("init dict", np.count_nonzero(dict_list_parallelepiped_vars[biogeoch_var][int(time)-year_interval[0]]))
            ...
            pres_list = ds['PRES' + '_' + dict_float_names[biogeoch_var]][:].data   #[0]  # list of value
            #print("pres list", pres_list)
            depth_list = []
            for pres in pres_list:
                depth_list.append(to_depth(pres, lat))

           
            #if biogeoch_var in float_var_list:
            biog_var = ds[dict_float_names[biogeoch_var]][:].data    #[0]
                ...
                if lon_max > lon > lon_min:
                    for depth in depth_list:
                        if depth_max > depth > depth_min:
                            ...
                            channel_index = np.where(depth_list == depth)[0][0]    #ho capito perchè lo ha usato, ma cmq non mi convince al 100%   
            ...
            print("select par end", np.count_nonzero(select_parallelepiped))
            print("end dict", np.count_nonzero(dict_list_parallelepiped_vars[biogeoch_var][int(time)-year_interval[0]]))

            np.save(os.getcwd() + "/dataset/float/" + str(2022) + "_" + str(2024) + "/saved_parallelepiped_biogeoch/" + biogeoch_var + "_parallelepiped.npy", dict_list_parallelepiped_vars[biogeoch_var][int(time)-year_interval[0]])     # [int(year_interval[i])-year_interval[0]])     #anceh qui prima era np.save
                            

    return


#potremmo ottimizzare un attimo la lista di variabili biogeochimiche --> generalizzare il codice

#list_data = pd.read_csv(float_path + '/Float_2022_Index.txt', header=None).to_numpy()[:, 0].tolist()
#list_datetime = pd.read_csv(float_path + '/Float_2022_Index.txt', header=None).to_numpy()[:, 3].tolist()
#print("len data", len(list_datetime))
...
#print("list data times", list_data_times)
...
#print(dict_list_parallelepiped_vars.keys())
#print(len(dict_list_parallelepiped_vars["P_l"][0]))
#print("int year", int(year_interval[i])-year_interval[0])
...
if kindof == "float_start":
   for i in range(1):
      insert_float_values(latitude_interval, longitude_interval, depth_interval, (2022, 2024), resolution, list_data_times,dict_list_parallelepiped_vars, float_var_lists)
...
if kindof == "float_hov":
  lat_limits = latitude_interval
  lon_limits = longitude_interval
  depth_limits = depth_interval
  lat_min, lat_max = lat_limits
  lon_min, lon_max = lon_limits
  depth_min, depth_max = depth_limits
  year_min, year_max = 2022, 2023
  w_res, h_res, d_res = resolution
  w = int((lat_max - lat_min) * constant_latitude / w_res + 1)
  h = int((lon_max - lon_min) * constant_longitude / h_res + 1)
  d = int((depth_max - depth_min) / d_res + 1)
  ...
  dict_float_names = dictionary_float_vars_names(list_biogeoch_vars)
  print("dict float names", dict_float_names)
  total_float_tensor = torch.zeros([d, len(list_data)])
  for i in range(np.size(list_data)):  # indexing on list_data and list_datetime also
    path_current_float = float_path + list_data[i]
    print("path current float", path_current_float, flush=True)
    ds = nc.Dataset(path_current_float)
    print("ds", ds)
    my_datetime = list_datetime[i]
    time = read_date_time_sat(my_datetime)
    print("time", time)
    if not 2022 < time < 2024:
      continue
    ...
    pres_list = ds['PRES' + '_' + dict_float_names["P_l"]][:].data 
    depth_list = []
    for pres in pres_list:
      depth_list.append(to_depth(pres, lat))
    print("depth list", depth_list)
    biog_var = ds[dict_float_names["P_l"]][:].data
    float_tensor = torch.zeros([d])
    if lat_max > lat > lat_min:
      if lon_max > lon > lon_min:
        for depth in depth_list:
          if depth_max > depth > depth_min:
