...
def generate_dataset_phase_1_saving(biogeoch_var, path_results, year, path_saving):
    ...
    #remove the tensors that I don't use anymore
    #del total_dataset 
    #save total_dataset_norm
    #n_dupl_base = len(os.listdir(path_saving + "/total_dataset_norm/" + str(biogeoch_var) + "/" + str(year) + "/" + "week_" + str(1) + "/"))
    #for i in range(len(total_dataset_norm)):
     #   i_week = list_duplicates_biogeoch_weeks[i]
      #  n_dupl = i % n_duplicates_biogeoch + n_dupl_base
       # tot_data_norm_path_sav = path_saving + "/total_dataset_norm/" + str(biogeoch_var) + "/" + str(year) + "/" + "week_" + str(i_week) + "/" + "duplicate_" + str(n_dupl)+ "/"
        #if not os.path.exists(tot_data_norm_path_sav):
         #   os.makedirs(tot_data_norm_path_sav)
        #torch.save(total_dataset_norm[i], tot_data_norm_path_sav + "tensor.pt")
        #write_list(transposed_duplicates_biogeoch_coordinates[i], tot_data_norm_path_sav + "file_transpose_latitudes.txt")
    return None
...
def generate_dataset_phase_2_saving(biogeoch_var, path_results_2, years, path_saving_2, land_sea_masks):
    ...
    for year in years:
        ...
        physics_train_dataset_2, list_physics_weeks_2 = re_order_weeks(list_physics_weeks_2, physics_train_dataset_2)
        biogeoch_float_train_dataset, float_list_weeks = re_order_weeks(float_list_weeks, biogeoch_float_train_dataset)
        #print("len list physics tensors", len(physics_train_dataset_2))
        #print("len list biogeoch tensors", len(biogeoch_float_train_dataset))
        #pop the last element since it is a void tensor
        #physics_train_dataset_2.pop(-1)  
        #list_physics_weeks_2.pop(-1)
        if year != 2020:  
    ...
    #print("len list float profiles coordinates", len(list_float_profiles_coordinates))
    #print("tensors with no float", [j for j in range(len(list_float_profiles_coordinates)) if len(list_float_profiles_coordinates[j]) == 0])
    sampled_list_float_profile_coordinates = [random.sample(float_profile_coord, int(0.4 * len(float_profile_coord))) for float_profile_coord in list_float_profiles_coordinates]
