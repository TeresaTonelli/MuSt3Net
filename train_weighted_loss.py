...
physics_train_dataset = get_list_model_tensor("physics_vars")
biogeoch_train_dataset, list_biogeoch_weeks = get_list_model_tensor_weeks(list_biogeoch_vars[0])

#physics_train_dataset = [physics_train_data.to(device) for physics_train_data in physics_train_dataset]  
...
#duplicates_biogeoch_train_dataset = [dupl_biog_tensor.to(device) for dupl_biog_tensor in duplicates_biogeoch_train_dataset]
duplicates_physics_train_dataset = extend_list(physics_train_dataset, n_duplicates_biogeoch)
#duplicates_physics_train_dataset = [dupl_phys_tensor.to(device) for dupl_phys_tensor in duplicates_physics_train_dataset]
...
#print("list duplicates biogeoch weeks", list_duplicates_biogeoch_weeks)
...
#biogeoch_train_dataset_fill = [fill_tensor_with_standard(biogeoch_tensor, land_sea_masks, standard_mean_values[list_biogeoch_vars[0]]) for biogeoch_tensor in duplicates_biogeoch_train_dataset]
#print("end fill")
#biogeoch_train_dataset_fill = [biog_tens_fill.to(device) for biog_tens_fill in biogeoch_train_dataset_fill]
#print("end fill")
...
for ep in range(epoch_c):
    ...
    for i in range(len(train_dataset)):
        ...
        #loss_completion = convolutional_network_weighted_loss(old_total_dataset[int(index_training[i] / n_duplicates_biogeoch)][:, :, :-1, :, 1:-1].float(), denormalized_NN_output.float(), land_sea_masks)
    ...
    if ep % snaperiod == 0:
        ...
        with torch.no_grad():
            ...
            for i_test in range(len(test_dataset)): 
                ...
                #loss_1c_test = convolutional_network_weighted_loss(old_total_dataset[int(index_testing[i_test] / n_duplicates_biogeoch)][:, :, :-1, :, 1:-1].float(), denormalized_testing_output.float(), land_sea_masks)
...
model_completion.eval()
with torch.no_grad():
    for i in range(len(test_dataset[0:10])):
        ...
        plot_difference_maps(torch.unsqueeze(old_total_dataset[int(index_testing[i] / n_duplicates_biogeoch)][:, :, :-1, :, 1:-1][:, 6, :, :, :], 1), denorm_testing_output, land_sea_masks, var, path_difference_map_1p)
...
physics_train_dataset_2 = get_list_model_tensor("physics_vars")
physics_train_dataset_2.pop(-1)
#physics_train_dataset_2 = [phys_tensor.to(device) for phys_tensor in physics_train_dataset_2]
...
#biogeoch_float_train_dataset = [biog_float_tensor.to(device) for biog_float_tensor in biogeoch_float_train_dataset]
...
#list_float_profiles_coordinates = [compute_profile_coordinates(float_tensor[:, 0:1, :, :, :].to(device)) for float_tensor in biogeoch_float_train_dataset]
...
#reduced_biogeoch_float_train_dataset = [remove_float(biogeoch_float_train_dataset[j], sampled_list_float_profile_coordinates[j]).to(device) for j in range(len(biogeoch_float_train_dataset))]
...
#fill_biogeoch_float_train_dataset = [fill_tensor_with_standard(reduced_biogeoch_float_train_dataset[i_float][:, 0:1, :, :, :], land_sea_masks, standard_mean_values[list_biogeoch_vars[0]]).to(device) for i_float in range(len(reduced_biogeoch_float_train_dataset))]
#fill_biogeoch_float_train_dataset = [fill_tensor_opt(reduced_biogeoch_float_train_dataset[i_float][:, 0:1, :, :, :], land_sea_masks, standard_mean_values[list_biogeoch_vars[0]]).to(device) for i_float in range(len(reduced_biogeoch_float_train_dataset))]
...
for ep in range(epoch_c_2):
    ...
    for i in range(len(train_dataset_2)):
        ...
        #loss_completion_2 = convolutional_network_float_weighted_loss(denorm_old_float_train_dataset[index_training_2[i]].float(), denormalized_output.float(), land_sea_masks, generate_float_mask(sampled_list_float_profile_coordinates[index_training_2[i]]).to(device))
        ...
        with torch.no_grad():
            ...
            for i_test in range(len(test_dataset_2)):
                ...
                denormalized_output = Denormalization(testing_output, torch.unsqueeze(torch.load(path_mean_std_2 + "/mean_tensor.pt")[:,6, :,:,:],1).to(device), 
                                                      torch.unsqueeze(torch.load(path_mean_std_2 + "/std_tensor.pt")[:,6,:,:,:],1).to(device)).to(device)
                #loss_1c_test_2 = convolutional_network_float_weighted_loss(denorm_old_float_train_dataset[index_testing_2[i_test]].float(), denormalized_output.float(), land_sea_masks, generate_float_mask(sampled_list_float_profile_coordinates[index_testing_2[i_test]]).to(device))  #convolutional_network_loss(test_data, testing_output, land_sea_masks)
...
with torch.no_grad():
    for i in range(len(test_dataset_2)):
        ...
        plot_difference_maps(torch.unsqueeze(old_total_dataset[int(index_testing_2[i] / n_duplicates_biogeoch)][:, :, :-1, :, 1:-1][:, 6, :, :, :], 1), denorm_testing_output_2, land_sea_masks, var, path_difference_map_2p)
...
with torch.no_grad():
    for i in range(len(test_dataset_2)):
        ...
        denorm_testing_output_1 = Denormalization(norm_testing_output_1, torch.unsqueeze(torch.load(path_mean_std_2 + "/mean_tensor.pt")[:,6,:,:,:],1), 
                                                  torch.unsqueeze(torch.load(path_mean_std_2 + "/std_tensor.pt")[:,6,:,:,:],1)) 
